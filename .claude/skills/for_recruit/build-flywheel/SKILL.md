---
name: build-flywheel-for-recruit
description: |
  ForRecruit特化版: 成長を加速させるフライホイールを設計する自律実行型スキル。社内エコシステム連携（Airシリーズモデル）を重視し、リクルート資産3種以上活用でLTV 3-5倍向上、クロスセル効果定量化（Airレジ→Airペイ 57%）を目標に設計します。

  ForRecruit固有の特徴:
  - エコシステム連携評価: 社内製品連携でChurn率1/2〜1/3
  - クロスセル効果定量化: Airレジ→Airペイ クロスセル率57%ベンチマーク
  - LTV向上目標: 単体製品比3-5倍（エコシステム連携効果）
  - 社内リソース活用: 営業網・データ資産・ブランドの統合活用
  - Recruit Product Research統合（15-20事例）
...
# Build Flywheel Skill (ForRecruit Edition)

成長を加速させるフライホイールを設計する自律実行型Skill。**ForRecruit特化版**では、社内エコシステム連携（Airシリーズモデル）を最重視し、リクルート資産活用による10倍優位性構築を目標とします。

---

## このSkillでできること

1. **成長エンジン選定**: Viral/Sticky/Paidから最適なエンジンを選択（社内エコシステム連携重視）
2. **フライホイール設計**: 成長サイクルの構造化（Mermaid図）
3. **エコシステム連携評価**: 社内製品連携でChurn率1/2〜1/3、LTV 3-5倍向上
4. **クロスセル効果定量化**: Airレジ→Airペイ 57%をベンチマーク
...
- 2026-01-02: ForRecruit特化版として作成、Recruit Product Research統合（15-20事例）
