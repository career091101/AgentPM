---
name: simulate-interview-for-startup
description: |
  ForStartup特化版: 仮想ペルソナとのインタビューをシミュレートする自律実行型スキル。LLMがペルソナになりきり、UVPの刺さり度を10の質問で40点満点スコアリング。課題深堀り（4U検証40点満点）、既存代替案評価、WTP（支払い意思）確認を実施し、インサイトを抽出します。

  ForStartup固有の特徴:
  - インタビュー数基準緩和（10-15人、社内ネットワーク・既存顧客基盤活用）
  - アーリーアダプター導入前提の評価基準（Geppo事例: リクルート1,200名先行運用）
  - 既存顧客基盤活用によるリクルーティング効率化（ホットペッパーグルメ営業網2,000名）
  - Seed調達準拠（Seed Stage-2対応）
...
# Simulate Interview Skill (ForStartup Edition)

仮想ペルソナとのインタビューをシミュレートする自律実行型Skill。**ForStartup特化版**では、社内ネットワーク・既存顧客基盤活用による低コスト・高品質検証を前提としています。
...
| **出力** | `Stock/programs/創業支援・新規事業開発（AIエージェント）/projects/Founder_Agent_ForStartup/documents/2_discovery/interview_simulation.md` |
| **次のSkill** | `/research-problem` → `/validate-cpf` |
| **ステージ** | CPF検証（Seed調達Seed Stage-2） |
...
| 実施期間 | 2-4週間 | **1-2週間** | スタートアップリソース活用で短期化 |
| 4Uスコア基準 | 28/40以上で合格 | **25/40以上で合格** | 内部検証前提での段階的検証 |

### アーリーアダプター導入オプション

ForStartupでは、自社/グループ先行導入による課題検証が推奨されます。
...
1. **リクルートDMPフォロー**:
   ...
   - **ForStartup教訓**: 社内ネットワーク活用でも定量調査必須、仮説検証を厳密化

2. **CODE.SCORE（CORP_F002）**:
   - **失敗要因**: エンジニア採用課題の過小評価、課題共通率38%
   - **教訓**: ニーズ検証の甘さ、競合優位性不足
   - **ForStartup教訓**: 既存顧客基盤との関連性を最優先で確認、技術力のみでは不足
...
- **User Research Count**: 成功製品平均35.2回、ForStartup推奨: **10-15回**（既存顧客基盤活用で高品質化）
- **Problem Commonality**: 成功製品平均72.9%、ForStartup推奨: **60%以上**（内部検証前提で緩和）
...
- **25-34点**: ✅ 良好（3-4U合格） → **ForStartup推奨: アーリーアダプター導入検討**
...
| **25-27点** | ✅ 良好（ForStartup特例） | 2-3U合格 | **アーリーアダプター導入推奨**、外部検証と併用 |
...
- スキル別推奨事例: /Users/yuichi/AIPM/aipm_v0/.claude/skills/_shared/case_reference_for_startup.md#skill-mapping-simulate-interview
- ヒアリング事例: /Users/yuichi/AIPM/aipm_v0/Stock/programs/創業支援・新規事業開発（AIエージェント）/projects/Founder_Research/documents/01_Legendary/ /Users/yuichi/AIPM/aipm_v0/Stock/programs/創業支援・新規事業開発（AIエージェント）/projects/Founder_Research/documents/03_VC_Backed/
...
```
User: /simulate-interview-for-startup

Skill:
# インタビューシミュレーション 自律実行開始（ForStartup Edition）
...
UVP刺さり度: 34/40点（✅ 良好）
4Uスコア: 30/40点（✅ 良好）
ForStartup適合性: 18/20点（✅ アーリーアダプター導入推奨）
...
**最終更新**: 2026-01-02
**作成者**: Claude Code
**ForStartup特化要素**: 5件の成功事例統合、アーリーアダプター導入オプション追加、適合性評価基準追加、Seed調達連携
