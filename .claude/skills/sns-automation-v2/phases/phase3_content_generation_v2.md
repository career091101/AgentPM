...
**URL参照データ**:
- Top 1記事のURL一覧: `{top1_urls}`
- 統一フォーマット: 「■ ソース」 + 空行 + URL一覧（箇条書き記号なし）
- URL取得は `extract_urls_for_top_article()` を使用
- フォーマット化は `format_url_section()` を使用
...
5. **品質チェックリスト7項目必須**:
   ...
   - 問いかけ終結（「あなたはどう思いますか？」）
6. **URL参照追加（新規）**:
   - **配置**: ツイート7（CTA）にURL一覧を統合
   - **文字数制約対応**: ツイート7 + URLが280文字以内に収まる場合は統合、超過する場合はツイート8を生成（URL専用）
   - **フォーマット**: 「■ ソース」見出し + 空行 + URL一覧（各行1URL、箇条書き記号なし）
   - **検証**: `check_character_limit()` でツイート7 + URL一覧が280文字以内か確認
...
5. 深掘り2: 反論への対応
6. 影響分析: 今後の展望
7. 行動喚起（CTA + URL統合）: 読者への問いかけ + URL一覧
   - **280文字以内の場合**: CTA本文 + 空行 + 「■ ソース」 + 空行 + URL一覧
   - **280文字超過の場合**: ツイート7はCTA本文のみ、ツイート8にURL専用ツイート生成
...
出力形式: JSON
{
  "tweets": [
    ...
    ...,
    {"order": 7, "content": "あなたはどう思いますか？\n\n■ ソース\n\nhttps://...", "char_count": 275}
  ],
  "url_placement": "integrated",  // or "separate" if ツイート8生成
  "total_tweets": 7,  // or 8 if URL専用ツイート追加
...
**URL参照データ**: Top 2記事のURL一覧 (`{top2_urls}`)を使用。ツイート7（CTA）にURL統合、文字数超過時はツイート8生成。

#### Top 3記事 → Xスレッド3（5-7ツイート）

同様のプロンプトでTop 3記事について生成。パターンはTop 1, Top 2と異なるものを選択。

**URL参照データ**: Top 3記事のURL一覧 (`{top3_urls}`)を使用。ツイート7（CTA）にURL統合、文字数超過時はツイート8生成。
...
4. **構成**:
   ...
   - CTA（1-2行）: 行動喚起・問いかけ
5. **URL参照追加（新規）**:
   - **配置**: 本文末尾（CTA後）にURL一覧を追加
   - **文字数制約対応**: 本文 + URLが500文字以内に収まる場合は統合、超過する場合は2投稿目を生成（URL専用）
   - **フォーマット**: 空行 + 「■ ソース」 + 空行 + URL一覧
   - **検証**: `check_character_limit()` で本文 + URL一覧が500文字以内か確認
...
6. **URL参照追加（新規）**:
   - **配置**: 最後の投稿（2投稿目 or 3投稿目）にURL一覧を追加
   - **文字数制約対応**: 最後の投稿 + URLが500文字以内に収まる場合は統合、超過する場合は追加投稿を生成（URL専用）
   - **フォーマット**: 空行 + 「■ ソース」 + 空行 + URL一覧
   - **検証**: `check_character_limit()` で最後の投稿 + URL一覧が500文字以内か確認
...
{
  "type": "single",
  "content": "Threads投稿本文 + URL一覧（500文字以内）",
  "char_count": 478,
  "url_placement": "integrated",  // or "separate" if 2投稿目生成
  "quality_check": {
    "具体的数値": true,
    "固有名詞": true,
    "問いかけ": true
...
【スレッド投稿の場合】
{
  ...
  "posts": [
    ...
    {"order": 2, "content": "2投稿目本文 + URL一覧（500文字以内）", "char_count": 495},
    ...
  ],
  "total_posts": 2,  // or 3 if URL専用投稿追加
  "url_placement": "integrated",  // or "separate" if 追加投稿生成
  "quality_check": {
    "具体的数値": true,
    "固有名詞": true,
    "問いかけ": true
...
**URL参照データ**: Top 2記事のURL一覧 (`{top2_urls}`)を使用。本文末尾またはスレッド最後の投稿にURL統合、文字数超過時は追加投稿生成。

#### Top 3記事 → Threads投稿3

同様のプロンプトでLinkedIn案3を基に生成。

**URL参照データ**: Top 3記事のURL一覧 (`{top3_urls}`)を使用。本文末尾またはスレッド最後の投稿にURL統合、文字数超過時は追加投稿生成。
