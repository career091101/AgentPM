#!/usr/bin/env python3
"""
X投稿（案3: OpenAI平均年収2.2億円）をLate APIで予約投稿 v2
...
def main():
    ...
    # 元となるX投稿URL（うみゆき_aiの投稿 - 仮URL、実際のURLに差し替えてください）
...
冷静ですか？"""
    ...
    # 予約投稿時刻: 2026-01-06 19:50 JST（修正）
    ...
    # リクエストボディ構築（修正版）
    request_body = {
        "content": tweets[0],  # 1ツイート目（URL + CTA含む）
    ...
    print("🚀 Late APIへ投稿中...")
    print(f"\n【1ツイート目プレビュー】")
    print(f"{tweets[0][:200]}...")
    print(f"\n【スレッド数】: {len(tweets[1:])}ツイート（2-7）")
    print()
    ...
    try:
        ...
        if response.status_code == 200 or response.status_code == 201:
            ...
            print()
            print("📝 確認事項:")
            print("   1. Late APIダッシュボードで7ツイート全て表示されているか確認")
            print("   2. 1ツイート目にURLが正しく含まれているか確認")
            print("   3. スレッド番号は自動付与されないため、必要に応じて手動追加")
        else:
            print("❌ 投稿失敗")
