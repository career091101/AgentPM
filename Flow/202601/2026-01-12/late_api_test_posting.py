...
def create_linkedin_test_post(env_vars: dict, scheduled_time: datetime) -> dict:
    """LinkedIn テスト投稿作成"""

    scheduled_str = scheduled_time.strftime("%Y-%m-%dT%H:%M:%S+09:00")

    content_text = """**AIエージェントの本質は「スキル」にある。**
...
def create_twitter_test_post(env_vars: dict, scheduled_time: datetime) -> dict:
    """X (Twitter) テスト投稿作成"""
...
def create_threads_test_post(env_vars: dict, scheduled_time: datetime) -> dict:
    """Threads テスト投稿作成"""
...
def main():
    ...
    try:
        ...
        print("="*70)
        linkedin_payload = create_linkedin_test_post(env_vars, scheduled_time)
        results["linkedin"] = post_to_late_api(linkedin_payload, env_vars["api_key"])
    ...
    try:
        ...
        print("="*70)
        twitter_payload = create_twitter_test_post(env_vars, scheduled_time + timedelta(minutes=5))
        results["twitter"] = post_to_late_api(twitter_payload, env_vars["api_key"])
    ...
    try:
        ...
        print("="*70)
        threads_payload = create_threads_test_post(env_vars, scheduled_time + timedelta(minutes=10))
        results["threads"] = post_to_late_api(threads_payload, env_vars["api_key"])
    ...
    # 結果をJSONファイルに保存
    output_path = Path("/Users/yuichi/agentpm/Flow/202601/2026-01-12/late_api_test_results.json")
