...
使い方: python fetch_batch.py <batch_number>
例: python fetch_batch.py 1
"""
...
# 33都府県を4バッチに分割
BATCHES = {
    1: ["青森県", "岩手県", "宮城県", "秋田県", "山形県", "福島県", "茨城県", "栃木県", "群馬県", "埼玉県"],
    2: ["千葉県", "神奈川県", "新潟県", "富山県", "石川県", "福井県", "山梨県", "長野県", "岐阜県", "静岡県"],
    3: ["三重県", "滋賀県", "京都府", "兵庫県", "奈良県", "和歌山県", "鳥取県", "島根県", "岡山県", "広島県"],
    4: ["山口県", "徳島県", "香川県"]
}
...
# メイン処理
if len(sys.argv) < 2:
    print("使い方: python fetch_batch.py <batch_number>")
    print(f"利用可能なバッチ: 1-{len(BATCHES)}")
    print("\nバッチ内容:")
    for batch_num, prefs in BATCHES.items():
        print(f"  Batch {batch_num}: {len(prefs)}都府県 ({prefs[0]}〜{prefs[-1]})")
    exit(1)

batch_number = int(sys.argv[1])

if batch_number not in BATCHES:
    print(f"❌ 無効なバッチ番号: {batch_number}")
    print(f"利用可能なバッチ: 1-{len(BATCHES)}")
    exit(1)

PREFECTURES = BATCHES[batch_number]

print("=" * 60)
print(f"Batch {batch_number}: {len(PREFECTURES)}都府県データ取得")
print("=" * 60)
print(f"対象: {PREFECTURES[0]}〜{PREFECTURES[-1]}")
print(f"クエリ数: {len(PREFECTURES)} × {len(SPECIALTIES)} = {len(PREFECTURES) * len(SPECIALTIES)}クエリ\n")
...
output_file = f"batch{batch_number}_pediatric_dental_{timestamp}.csv"
