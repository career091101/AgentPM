...
echo "実行日時: $(date '+%Y-%m-%d %H:%M:%S')" >> "$OUTPUT_FILE"
echo "調査対象: 全ケーススタディ" >> "$OUTPUT_FILE"
echo "" >> "$OUTPUT_FILE"
...
if [[ "$warning_count" -eq 0 ]]; then
  echo "✅ すべての指標が目標を達成しています" >> "$OUTPUT_FILE"
fi
...
for tier_dir in documents/*/; do
  ...
  while IFS= read -r file; do
    if grep -q "interview_count: null" "$file" 2>/dev/null; then
      ...
      echo "- ⚠️  $filename" >> "$OUTPUT_FILE"
      has_missing=true
    fi
  done < <(find "$tier_dir" -name "*.md" -type f | sort)

  if [[ "$has_missing" == "false" ]]; then
    echo "✅ すべてのケースにinterview_count記載あり" >> "$OUTPUT_FILE"
  fi
  echo "" >> "$OUTPUT_FILE"
  ...
  if [[ "$has_missing" == "false" ]]; then
    echo "✅ すべてのケースにproblem_commonality記載あり" >> "$OUTPUT_FILE"
  fi
  echo "" >> "$OUTPUT_FILE"
...
- [ ] 01_Legendary全件のinterview_count補完
- [ ] 01_Legendary全件のproblem_commonality補完
- [ ] 品質スコアリング実施（90点以上を目標）
- [ ] 新規調査時は100%記載を徹底
