...
```yaml
interview_count: 50  # Brian Chesky (Airbnb): "We interviewed 50 hosts"
interview_count: 100  # Dharmesh Shah (HubSpot): "We talked to over 100 companies"
interview_count: 20  # Sahil Lavingia (Gumroad): "I interviewed 20 creators"
```

**ソース記載例**:
```yaml
primary_sources:
  - "Brian Chesky YC Interview 2013: 'We lived with our hosts and did 50 in-person interviews'"
...
例:
- Uber: 都市部居住者の80%が「車を持たない、または持ちたくない」
- HubSpot: 中小企業の90%が「営業とマーケの連携に課題」
- Notion: ナレッジワーカーの70%が「ツールの分散に不満」
...
```yaml
problem_commonality: 80  # Gartner調査: 都市部の80%が公共交通の不便さを感じる
problem_commonality: 90  # Forrester: 中小企業の90%が営業効率化に課題
problem_commonality: 75  # Statista: フリーランサーの75%が案件探しに週10時間以上
```
...
```yaml
primary_sources:
  - "Brian Chesky YC Interview 2013"
...
```yaml
interview_count: 50  # Brian Chesky公式ブログ（最も信頼性高）
...
2. **業界標準値の活用**
   - B2B SaaS: interview_count=25, problem_commonality=65%
