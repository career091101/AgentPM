#!/usr/bin/env python3
"""Retry failed articles from retry_failures.jsonl"""
import json
import sys
...
def main():
    ...
    final_failures_path = output_dir / "logs" / "final_failures.jsonl"

    if not retry_failures_path.exists():
        print("No retry_failures.jsonl found")
        return

    failed_urls = []
    with retry_failures_path.open("r", encoding="utf-8") as f:
        for line in f:
            line = line.strip()
            if not line:
                continue
    ...
    for idx, url in enumerate(failed_urls, 1):
        try:
            ...
            with final_failures_path.open("a", encoding="utf-8") as f:
