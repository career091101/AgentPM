...
# 削除対象の投稿ID（先ほど予約した単一投稿）
OLD_POST_ID = "695a54ba72ad0320af1346cc"
...
# Step 1: 既存投稿を削除
print('=' * 50)
print('🗑️  Deleting old Threads post (single format)...')
print(f'   Post ID: {OLD_POST_ID}')

delete_url = f'https://getlate.dev/api/v1/posts/{OLD_POST_ID}'
response = requests.delete(delete_url, headers=headers)

if response.status_code in [200, 204]:
    print('✅ Old post deleted successfully!')
else:
    print(f'⚠️  Delete warning: {response.status_code}')
    print(f'   Response: {response.text}')
    print('   Continuing with new post...')
...
if response.status_code in [200, 201]:
    ...
    result_file = os.path.join(data_dir, 'post_result_threads_thread_20260105200000.json')
    with open(result_file, 'w', encoding='utf-8') as f:
        json.dump({
            'old_post_id': OLD_POST_ID,
            'new_post_id': post_id,
            'format': 'thread',
            'thread_count': len(threads_thread),
