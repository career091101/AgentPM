#!/usr/bin/env python3
"""
d_1d2d メンバーシップ記事抽出スクリプト
https://note.com/d_1d2d/membership/notes

元スクリプト: markelabo_note_fetcher.py
"""
import argparse
import json
import os
...
# クリエイターAPIを使用（membershipフィルタ付き）
MEMBERSHIP_API_URL = "https://note.com/api/v2/creators/d_1d2d/contents"
MEMBERSHIP_KEY = "d_1d2d"
EXPECTED_NOTE_COUNT = None  # 実行時に取得

# 記事収集範囲（2025年1月〜）
ARCHIVE_START_YM = "2025-01"  # メンバーシップ開始月
ARCHIVE_END_YM = "2026-01"    # 最新記事月
...
def collect_article_urls_via_api(session, start_ym, end_ym, limit=None, log_path=None):
    """
    note APIからメンバーシップの記事URLリストを取得
    v2 API使用: kind=note&page=N 形式
    """
    urls = []
    seen = set()
    note_count = None
    ...
    # v2 APIは月別フィルタなし、ページングのみ
    page = 1
    while True:
        url = f"{MEMBERSHIP_API_URL}?kind=note&page={page}"
        try:
            payload = fetch_json(session, url, sleep_s=1.0, retries=3, backoff_s=2.0, log_path=log_path)
        ...
        if note_count is None:
            note_count = data.get("totalCount") or data.get("total_count")

        if not contents:
            break

        for item in contents:
            # v2 APIではitemが直接noteオブジェクト
            note_url = item.get("noteUrl") or item.get("note_url") or item.get("key")
            if not note_url:
                continue

            # URLの正規化
            if not note_url.startswith("http"):
                # keyの場合は /d_1d2d/n/{key} 形式
                if not note_url.startswith("/"):
                    note_url = f"/d_1d2d/n/{note_url}"
                full = urljoin("https://note.com", note_url)
            else:
                full = note_url

            # d_1d2d の記事のみを収集
            if "/d_1d2d/n/" not in full:
                continue
        ...
        # ページング終了判定
        is_last = data.get("isLastPage") or data.get("is_last_page", False)
        if is_last or len(contents) == 0:
            break
        page += 1
...
def main():
    parser = argparse.ArgumentParser(description="d_1d2d メンバーシップ記事抽出ツール")
    parser.add_argument("--login", action="store_true", help="ブラウザでログインしてcookiesを保存")
    parser.add_argument("--limit", type=int, default=0, help="取得する記事数（0=全件）")
    parser.add_argument("--output", type=str, default="../data/d_1d2d_articles", help="出力ディレクトリ")
    parser.add_argument("--cookies", type=str, default="../data/cookies/d_1d2d_cookies.json", help="Cookiesファイルパス")
    parser.add_argument("--cookies-txt", type=str, help="Netscape cookies.txtパス")
    parser.add_argument("--start-ym", type=str, default="2025-01", help="収集開始年月（YYYY-MM）")
    parser.add_argument("--end-ym", type=str, default="2026-01", help="収集終了年月（YYYY-MM）")
    ...
    start_ym = args.start_ym if hasattr(args, 'start_ym') else ARCHIVE_START_YM
    end_ym = args.end_ym if hasattr(args, 'end_ym') else ARCHIVE_END_YM

    print(f"📚 d_1d2d メンバーシップ記事収集開始")
    print(f"   URL: {MEMBERSHIP_URL}")
    print(f"   期間: {start_ym} ～ {end_ym}")
    print()
    ...
    print(f"✓ 記事URL収集完了: {len(article_urls)}件")
