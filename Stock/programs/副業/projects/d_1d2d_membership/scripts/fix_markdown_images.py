...
def clean_markdown_images(md_path):
    ...
    # data URI形式の画像参照を削除（リンク付き画像を先に処理）
    # パターン1: [![alt](data:image/...)](link) - より貪欲にマッチ
    content = re.sub(r'\[!\[[^\]]*\]\(data:image/[^\)]*\)[^\]]*\]\([^\)]+\)', '', content)

    # パターン2: ![alt](data:image/...)
    content = re.sub(r'!\[[^\]]*\]\(data:image/[^\)]*\)', '', content)

    # パターン3: 残った data URI の断片を削除
    content = re.sub(r'\[[^\]]*data:image/[^\]]*\]', '', content)
    content = re.sub(r'\[[^\]]*%3Csvg[^\]]*\]', '', content)

    # パターン4: URL-encodedなSVG断片を削除（例: [%22%20d%3D%22M-100...)](link)
