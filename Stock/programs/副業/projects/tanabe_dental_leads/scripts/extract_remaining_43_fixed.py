#!/usr/bin/env python3
"""
残り43都府県データ取得（本番版、4,100件）

実行フロー:
1. 43都府県から4,100件取得（40分）
                ...
                if places_result['status'] == 'OK':
                    for place in places_result['results']:
                        ...
                        try:
                            ...
                            print(f"⚠️ 詳細取得エラー ({place_id}): {str(e)[:100]}")
                            continue

                time.sleep(0.2)  # API制限対策

            except Exception as e:
                print(f"⚠️ 検索エラー ({search_query}): {str(e)[:100]}")
                continue
...
def save_batches(clinics, start_batch_num=21):
    ...
    for i in range(total_batches):
        ...
        if (i + 1) % 10 == 0:
            print(f"✓ バッチ{batch_num:03d}まで保存完了 ({i+1}/{total_batches})")

    print(f"✅ 全{total_batches}バッチ保存完了")
    return saved_files
...
def main():
    ...
    print(f"  2. merge_all_batches.py で全バッチを統合して5,100件完成")
